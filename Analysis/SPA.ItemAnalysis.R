...
spaResults = merge(spaAll %>% mutate(newID = rownames(.)
                                     , across(itemCols, ~ifelse(is.na(.), 0, .)))
                   , pptDetails %>% select(newID, starts_with("sp_")) %>% 
                     # rename(newID = Child_ID, sp_age=sp.age) %>% 
                     mutate(across(starts_with("sp_"), as.numeric))
                   , by = "newID") %>% 
  # rowwise() %>% 
  mutate(sp_alt = rowSums(select(., itemCols))
         , sp_irr_alt = rowSums(select(., itemCols[1:29]))
         , sp_reg_alt = rowSums(select(., itemCols[1:29+29]))
         , sp_nw_alt = rowSums(select(., itemCols[1:29+58]))
         )
...
ggplot(spa7plus, aes(x=sp_age, y=sp_alt))+
  ...
  # geom_smooth()+
  geom_line(data=qrMod, aes(colour=NULL), size=1, col="blue")+
  theme_minimal()+
  labs(x="Age in months", y="Score (of 87)"
       , title="16th Percentile Performance", subtitle="as a function of age")

# So, let's flag the poorest 16% of SPA spellers (age adjusted)

spa7plus = spa7plus %>% 
  mutate(sub16p = ifelse(sp_alt < qr.fit, 1, 0))
# table(spa7plus$sub16p)/nrow(spa7plus)
...
nSub16 = sum(spa7plus$sub16p)
spa7roc.overall = spa7plus %>% 
  arrange(screener) %>% 
  mutate(detection = cumsum(sub16p)/nSub16) %>% 
  group_by(screener) %>% 
...
ggplot(spa7roc.overall, aes(x=screener, y=detection))+
  geom_line(col="red")+
  ylab("Detect sub 16th percentile") + xlab("Screener threshold")+
  theme_minimal()

# Would need to apply a rule of 17 or less to catch 100% of sub16ps

### 7.4.2 Thresholds by Age ####
spa7detection.age = spa7plus %>% 
  group_by(ageCat, screener) %>% 
  summarise(detection=sum(sub16p)) %>% #slice(1:20) %>% data.frame()
  # ungroup(screener) %>% 
...
ggplot(spa7detection.age, aes(x=screener, y=detection, col=ageCat))+
  geom_line()+
  labs(title = "Screener's rate of detecting Full SPA 16th percentile kids"
       , subtitle = "by ages and threshold score"
       , colour = "Age Group"
       , x = "Screener threshold"
       , y = "Detect sub 16th percentile") +
  theme_minimal()+
...
# sub 8;5: 7 or less indicates follow up
# 8;6 - 9;5: 8 or less
# 9;5 - 10;5: 11 or less
# 10;6 plus: 17 or less
...
# age <= 7;5: 40% false positives
# - 8;5: 15%
# - 9;5: 10-12%
# - 10;5: 5-10%
# 10;6 +: 35-40% false positives 
...
# whether a kid is likely to fall in the "sub16p" category.

spa7tetrachoric = cor(spa7plus %>% select(all_of(itemCols)), spa7plus$sub16p) %>% data.frame %>% 
  rename(cor = ".") %>% arrange(cor) %>% slice(1:20)

## 8.1 by age ####
spa7tetrachoric.age = by(spa7plus, INDICES=spa7plus$ageCat
   , function(x) cor(x %>% select(all_of(itemCols)), x$sub16p) %>% data.frame %>% 
...
spa7screeners=apply(
  ...
  , function(x) {
    spa7plus %>% 
      ...
      select(newID, screener, ageCat, sub16p)
  }) %>% bind_cols() %>%
  rename(newID = newID...1, sub16p=sub16p...4, ageCat = ageCat...3
         , screener7 = screener...2, screener8 = screener...6
         , screener9 = screener...10, screener10 = screener...14
         , screener11 = screener...18) %>% 
  select(newID, ageCat, sub16p, starts_with("screener")
...
by(spa7screeners, INDICES=spa7screeners$ageCat
   , FUN=function(x) cor(x$sub16p, x$screener.age))
...
# 8;6 - 9;5: 12 or less
# 9;5 - 10;5: 14 or less
# 10;6 plus: 16 or less
# 7;6-8;5 has a lower threshold than the younger <=7;5. 
# I would be inclined to adopt the same threshold for all kids up to 8;5 – 
# though in the data, this would increase the false positives for <=7;5 to over
# 17,7%
...
# 7;5: virtually nil (seems to have caught one kid.)
# 8;5: around 15%
# 9;5: <5%
# 10;5: <5%
# 10;6+: 20-25%

### this works quite well, really. Perfect sensitivity, with relatively good
# specificity, especially for a screener.
...
#### 9.1.1.1 QR for IDing "16th percentile" ####

# ggplot(spa7plus, aes(x=sp_alt, y=ability, col=sp_age))+geom_point()
spa7plus$sp_irr_logit = qlogis((spa7plus$sp_irr_alt+1)/31)
spa7.qr_irr = rq(sp_irr_logit~sp_age, spa7plus, tau=.16)
...
ggplot(spa7plus, aes(x=sp_age, y=sp_irr_alt))+
  ...
  labs(x="Age in months", y="Score (of 29)"
       , title="16th Percentile Performance (Irregs)", subtitle="as a function of age")

# So, let's flag the poorest 16% of SPA spellers (age adjusted)

spa7plus = spa7plus %>% 
  mutate(sub16p.irr = ifelse(sp_irr_alt < qr_irr.fit, 1, 0))
# table(spa7plus$sub16p.irr)/nrow(spa7plus)

#### 9.1.1.2 Sensitivity ####

spa7screeners = spa7screeners %>% 
  merge(spa7plus %>% select(newID, sub16p.irr))

spa7detection.agescreeners.irr = spa7screeners %>% 
  group_by(ageCat, screener.age) %>% 
  summarise(detection.irr=sum(sub16p.irr)) %>% #slice(1:20) %>% data.frame()
  # ungroup(screener) %>% 
...
ggplot(spa7detection.agescreeners.irr
  ...
  geom_line()+
  labs(title = "Screener's rate of detecting Irregular SPA 16th percentile kids"
       , subtitle = "by ages and threshold score"
       , colour = "Age Group"
       , x = "Screener threshold"
       , y = "Detect sub 16th percentile") +
...
#### 9.1.2.1 QR for IDing "16th percentile" ####

spa7plus$sp_nw_logit = qlogis((spa7plus$sp_nw_alt+1)/31)
(spa7.qr_nw = rq(sp_nw_logit~sp_age, spa7plus, tau=.16)) %>% summary
...
ggplot(spa7plus, aes(x=sp_age, y=sp_nw_alt))+
  ...
  theme_minimal()+
  labs(x="Age in months", y="Score (of 29)"
       , title="16th Percentile Performance (Regs)", subtitle="as a function of age")

# So, let's flag the poorest 16% of SPA spellers (age adjusted)

spa7plus = spa7plus %>% 
  mutate(sub16p.nw = ifelse(sp_nw_alt < qr_nw.fit, 1, 0))
# table(spa7plus$sub16p.nw)/nrow(spa7plus)

#### 9.1.2.2 Sensitivity ####

spa7screeners = spa7screeners %>% 
  merge(spa7plus %>% select(newID, sub16p.nw))

spa7detection.agescreeners.nw = spa7screeners %>% 
  group_by(ageCat, screener.age) %>% 
  summarise(detection.nw=sum(sub16p.nw)) %>% #slice(1:20) %>% data.frame()
  # ungroup(screener) %>% 
...
ggplot(spa7detection.agescreeners.nw
  ...
  geom_line()+
  labs(title = "Screener's rate of detecting NW SPA 16th percentile kids"
       , subtitle = "by ages and threshold score"
       , colour = "Age Group"
       , x = "Screener threshold"
       , y = "Detect sub 16th percentile") +
...
#### 9.1.3.1 QR for IDing "16th percentile" ####

spa7plus$sp_reg_logit = qlogis((spa7plus$sp_reg_alt+1)/31)
(spa7.qr_reg = rq(sp_reg_logit~sp_age, spa7plus, tau=.16)) %>% summary
...
ggplot(spa7plus, aes(x=sp_age, y=sp_reg_alt))+
  ...
  theme_minimal()+
  labs(x="Age in months", y="Score (of 29)"
       , title="16th Percentile Performance (Regular)", subtitle="as a function of age")

# So, let's flag the poorest 16% of SPA spellers (age adjusted)

spa7plus = spa7plus %>% 
  mutate(sub16p.reg = ifelse(sp_reg_alt < qr_reg.fit, 1, 0))

#### 9.1.3.2 Sensitivity ####

spa7screeners = spa7screeners %>% 
  merge(spa7plus %>% select(newID, sub16p.reg))

spa7detection.agescreeners.reg = spa7screeners %>% 
  group_by(ageCat, screener.age) %>% 
  summarise(detection.reg=sum(sub16p.reg)) %>% #slice(1:20) %>% data.frame()
  # ungroup(screener) %>% 
...
ggplot(spa7detection.agescreeners.reg
  ...
  geom_line()+
  labs(title = "Screener's rate of detecting Regular SPA 16th percentile kids"
       , subtitle = "by ages and threshold score"
       , colour = "Age Group"
       , x = "Screener threshold"
       , y = "Detect sub 16th percentile") +
...
# identify the 16th percentile. Instead we can just use 1 SD below the mean,
# which in this case is any score of 85 or less.

spaMorphWiat = spaMorphWiat %>% 
  mutate(WIAT.sub16p = ifelse(WIAT.ss <=85, 1, 0))

WIATdetection.agescreeners = spaMorphWiat %>% 
  group_by(ageCat, screener.age) %>% 
  summarise(detection=sum(WIAT.sub16p, na.rm=T)) %>% #slice(1:20) %>% data.frame()
  # ungroup(screener) %>% 
  mutate(detection=cumsum(detection)/sum(detection)) %>% 
  ungroup() %>% 
  arrange(ageCat, screener.age)
...
#### 9.3.1.1 QR for IDing "16th percentile" ####
...
(morph.qr_tot = rq(morph_logit~sp_age, spaMorphWiat, tau=.16)) %>% summary
...
ggplot(spaMorphWiat, aes(x=sp_age, y=Morph.tot))+
  ...
  theme_minimal()+
  labs(x="Age in months", y="Score (of 29)"
       , title="16th Percentile Performance (Morph Total)", subtitle="as a function of age")

# So, let's flag the poorest 16% of SPA spellers (age adjusted)

spaMorphWiat = spaMorphWiat %>% 
  mutate(sub16p.morph.tot = ifelse(Morph.tot < morph.fit, 1, 0))

ggplot(spaMorphWiat, aes(x=sp_age, y=Morph.tot, col=sub16p.morph.tot))+
  geom_point()+
  # geom_smooth()+
  geom_line(data=qrMod.morph.tot, aes(colour=NULL), size=1, col="blue")+
  theme_minimal()+
...
morphScreeners = spa7screeners %>% 
  ...
  merge(spaMorphWiat %>% select(newID, sub16p.morph.tot))
...
Morphdetection.agescreeners.morph.tot = morphScreeners %>% 
  group_by(ageCat, screener.age) %>% 
  summarise(detection.morph=sum(sub16p.morph.tot)) %>% #slice(1:20) %>% data.frame()
  # ungroup(screener) %>% 
...
ggplot(Morphdetection.agescreeners.morph.tot
  ...
  geom_line()+
  labs(title = "Screener's rate of detecting Morph Total 16th percentile kids"
       , subtitle = "by ages and threshold score"
       , colour = "Age Group"
       , x = "Screener threshold"
       , y = "Detect sub 16th percentile") +
...
#### 9.3.2.1 QR for IDing "16th percentile" ####

spaMorphWiat$morph.rw_logit = qlogis((spaMorphWiat$Morph.rw+1)/29)
(morph.qr_rw = rq(morph.rw_logit~sp_age, spaMorphWiat, tau=.16)) %>% summary
...
ggplot(spaMorphWiat, aes(x=sp_age, y=Morph.rw))+
  ...
  labs(x="Age in months", y="Score (of 27)"
       , title="16th Percentile Performance (Morph Words)", subtitle="as a function of age")

# So, let's flag the poorest 16% of SPA spellers (age adjusted)

spaMorphWiat = spaMorphWiat %>% 
  mutate(sub16p.morph.rw = ifelse(Morph.rw < morph.rw.fit, 1, 0))

ggplot(spaMorphWiat, aes(x=sp_age, y=Morph.rw, col=sub16p.morph.rw))+
  geom_point()+
  # geom_smooth()+
  geom_line(data=qrMod.morph.rw, aes(colour=NULL), size=1, col="blue")+
  theme_minimal()+
...
morphScreeners = morphScreeners %>% 
  ...
  #select(newID, ageCat, starts_with("screener")) %>% 
  merge(spaMorphWiat %>% select(newID, sub16p.morph.rw))

# Morphdetection.agescreeners.morph.rw = morphScreeners %>% 
#   group_by(ageCat, screener.age) %>% 
#   summarise(detection.morph.rw=sum(sub16p.morph.rw)) %>% #slice(1:20) %>% data.frame()
#   # ungroup(screener) %>% 
...
ggplot(Morphdetection.agescreeners.morph.rw
  ...
  geom_line()+
  labs(title = "Screener's rate of detecting Morph Words 16th percentile kids"
       , subtitle = "by ages and threshold score"
       , colour = "Age Group"
       , x = "Screener threshold"
       , y = "Detect sub 16th percentile") +
...
#### 9.3.3.1 QR for IDing "16th percentile" ####

spaMorphWiat$morph.ps_logit = qlogis((spaMorphWiat$Morph.ps+1)/28)
(morph.qr_ps = rq(morph.ps_logit~sp_age, spaMorphWiat, tau=.16)) %>% summary
...
spaMorphWiat = spaMorphWiat %>% 
  mutate(sub16p.morph.ps = ifelse(Morph.ps < morph.ps.fit, 1, 0))

ggplot(spaMorphWiat, aes(x=sp_age, y=Morph.ps, col=sub16p.morph.ps))+
  geom_point()+
  # geom_smooth()+
  geom_line(data=qrMod.morph.ps, aes(colour=NULL), size=1, col="blue")+
  theme_minimal()+
  labs(x="Age in months", y="Score (of 29)"
       , title="16th Percentile Performance (Morph Pseudowords)", subtitle="as a function of age")
...
morphScreeners = morphScreeners %>% 
  ...
  #select(newID, ageCat, starts_with("screener")) %>% 
  merge(spaMorphWiat %>% select(newID, sub16p.morph.ps))

# Morphdetection.agescreeners.morph.ps = morphScreeners %>% 
#   group_by(ageCat, screener.age) %>% 
#   summarise(detection.morph.ps=sum(sub16p.morph.ps)) %>% #slice(1:20) %>% data.frame()
#   # ungroup(screener) %>% 
...
ggplot(Morphdetection.agescreeners.morph.ps
  ...
  geom_line()+
  labs(title = "Screener's rate of detecting Morph Pseudowords 16th percentile kids"
       , subtitle = "by ages and threshold score"
       , colour = "Age Group"
       , x = "Screener threshold"
       , y = "Detect sub 16th percentile") +
...
#### 9.4.1.1 QR for IDing "16th percentile" ####
...
#### 9.4.2.1 QR for IDing "16th percentile" ####
...
#### 9.4.3.1 QR for IDing "16th percentile" ####
...
## 10.2 tetrachoric "16th percentile" ####
...
ageAdj16thP = spaMorphWiat %>% 
  select(newID, sp_age, contains("sub16p")) %>% 
  rename(
    Age = sp_age
    , SPA.total = sub16p, SPA.reg = sub16p.reg, SPA.irr = sub16p.irr
    , SPA.nw = sub16p.nw
    , Morph.total = sub16p.morph.tot, Morph.rw = sub16p.morph.rw
    , Morph.ps = sub16p.morph.ps
    , WIAT = WIAT.sub16p
  ) %>% 
  select(newID, Age, SPA.total, SPA.reg, SPA.irr, SPA.nw
         , Morph.total, Morph.rw, Morph.ps, WIAT)
cor(ageAdj16thP %>% select(-newID), use="pairwise.complete")
