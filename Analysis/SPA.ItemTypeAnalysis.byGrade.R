...
par(mfrow=c(2,3))
for(type in names(spaYrItemTypeFPRs)){
  png(filename=paste("FPRs", type, "png", sep="."), width=960)
  for(yr in names(spaYrItemTypeFPRs[[type]]))
    (spaYrItemTypeFPRs[[type]][[yr]]+ggtitle(paste(type, yr)))
  dev.off()
}
